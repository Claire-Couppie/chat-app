...
const fakeMessageListContainer = [...Array(100).keys()].map(() => {
...
export default async (page, perPage) => {
    const messages = fakeMessageListContainer.slice(page * perPage, (page+1) * perPage).reverse();
    return { messages, total: fakeMessageListContainer.length };
}
